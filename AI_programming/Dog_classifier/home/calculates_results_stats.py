...
# PURPOSE: To Create a function calculates_results_stats that calculates the 
...
def calculates_results_stats(results_dic):
    ...
    for idx in results_dic:
        ...
        if (sum(results_dic[idx][3:]) == 2):
            n_correct_dogs += 1
       
        if (sum(results_dic[idx][3:]) == 0):
            n_correct_notdogs += 1
        if results_dic[idx][3] == 1:
            n_dogs_img += 1
        if results_dic[idx][3] == 0:
            n_notdogs_img += 1
        if (results_dic[idx][3] == 1 and results_dic[idx][2] == 1):
            n_correct_breed += 1
        if results_dic[idx][2] == 1:
            n_match += 1

    if n_dogs_img:
        pct_correct_dogs = ((n_correct_dogs * 1.0) / n_dogs_img) * 100
        pct_correct_breed = ((n_correct_breed * 1.0) / n_dogs_img) * 100
    ...
    if n_images:
        pct_label_matches = ((n_match * 1.0) / n_images) * 100
    else:
        pct_label_matches = 0
    if n_notdogs_img:
        pct_correct_notdogs = ((n_correct_notdogs * 1.0) / n_notdogs_img) * 100
    else:
